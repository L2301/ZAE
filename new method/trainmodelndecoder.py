...
class JointTrainingDataset(Dataset):
    ...
    def __init__(self, encoder, embedding, dataset_path, seq_length=4, max_samples=100000, device='cpu'):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'original_embeddings': tok_emb.cpu().squeeze(0)
        }
...
def train_joint(
    ...
    """Train decoder and GPT jointly."""
    
    run_id = f"joint_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
    ...
    # Create frozen reference GPT for ground truth
    gpt_core_reference, _ = load_gpt_model(gpt_checkpoint_path)
    gpt_core_reference.to(device)
    gpt_core_reference.eval()
    for param in gpt_core_reference.parameters():
        param.requires_grad = False
    ...
    for epoch in range(n_epochs):
        epoch_losses = {'total': 0, 'router': 0, 'recon': 0}
        
        pbar = tqdm(dataloader, desc=f"Epoch {epoch+1}/{n_epochs}")
        for batch in pbar:
            compressed = batch['compressed'].to(device)  # (B, 768)
            input_ids = batch['input_ids'].to(device)  # (B, seq_len)
            original_emb = batch['original_embeddings'].to(device)  # (B, seq_len, 768)
            ...
            # Ground truth: original sequence through reference GPT
            with torch.no_grad():
                full_emb = embedding(input_ids)  # (B, seq_len, 768)
                target_hidden = gpt_core_reference(full_emb)  # (B, seq_len, 768)
            
            # Get logits for both
            pred_logits = lm_head(reconstructed)  # (B, seq_len, vocab_size)
            target_logits = lm_head(target_hidden)  # (B, seq_len, vocab_size)
            
            # KL divergence loss
            pred_log_probs = F.log_softmax(pred_logits, dim=-1)
            target_probs = F.softmax(target_logits, dim=-1)
            recon_loss = F.kl_div(pred_log_probs, target_probs, reduction='batchmean')
            
            # Total loss
            total_loss = router_weight * router_loss + recon_weight * recon_loss
        ...
        print(f"  Recon Loss: {epoch_losses['recon']/n_batches:.4f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='train/joint')
